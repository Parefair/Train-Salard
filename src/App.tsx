...
function App() {
  ...
  const englishToThaiMap: { [key: string]: string } = {
    ...
    'o': 'น', 'p': 'ย', '[': 'บ', ']': 'ล', 'a': 'ฟ', 's': 'ห', 'd': 'ก', 'f': 'ด', 'g': 'เ', 'h': '้',
    'j': '่', 'k': 'า', 'l': 'ส', ';': 'ว', '\'': 'ง', 'z': 'ผ', 'x': 'ป', 'c': 'แ', 'v': 'อ', 'b': 'ิ',
    'n': 'ื', 'm': 'ท', ',': 'ม', '.': 'ใ', '/': 'ฝ',
    '!': '+', '@': '๑', '#': '๒', '$': '๓', '%': '๔', '^': 'ู', '&': '฿', '*': '๕', '(': '๖', ')': '๗',
    '_': '๘', '+': '๙', 'Q': '๐', 'W': '"', 'E': 'ฎ', 'R': 'ฑ', 'T': 'ธ', 'Y': 'ํ', 'U': '๊', 'I': 'ณ',
    'O': 'ฯ', 'P': 'ญ', '{': 'ฐ', '}': ',', 'A': 'ฤ', 'S': 'ฆ', 'D': 'ฏ', 'F': 'โ', 'G': 'ฌ', 'H': '็',
    'J': '๋', 'K': 'ษ', 'L': 'ศ', ':': 'ซ', '"': '.', 'Z': '(', 'X': ')', 'C': 'ฉ', 'V': 'ฮ', 'B': 'ฺ',
    'N': '์', 'M': '?', '<': 'ฒ', '>': 'ฬ', '?': 'ฦ'
  };
  ...
  const translate = (): void => {
    ...
    for (let char of temp) {
      ...
      if (char === ","){
        if (englishToThaiMap[last] || (last === " " && thaiToEnglishMap[temp2[len-2]])){
          temp2 += "ม"
        }
        else {
          temp2 += "}"
        ...
        <div className='flex flex-col gap-12'>
          <div className='flex basis-1/4 justify-between'>
            <div className='flex flex-col items-start gap-2'>
              <h1 className='text-white text-[64px] font-medium'>เทรน Sa-lard</h1>
              <div className='flex flex-col gap-1 items-start'>
                <p className='text-2xl text-purple font-medium'>คุณลืมเปลี่ยนภาษาก่อนกดพิมพ์ใช่ไหม?</p>
                <p className='text-2xl text-purple font-medium'>ให้เทรน Sa-lard เป็นตัวช่วยของคุณ</p>
              </div>
            </div>
            <div>
          ...
          <div className='basis-3/4'>
            <div className='flex flex-row gap-4 h-[414px]'>
              <div className='basis-2/5 w-full flex flex-col items-start'>
                <label  htmlFor="input" 
                        className='text-3xl text-white pb-2'>
                  ข้อความก่อน debut
                </label>
                <textarea  
                        name="input"
